...
/*   Updated: 2022/11/24 13:52:03 by sperez-s         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	paint(t_vars vars, unsigned int (*coloring)(int, int), int (*math)(t_cn, int))
{
	...
	while (i < vars.screen.size_x)
	{
		...
		{
			curr_n.r = initial_n.r + vars.screen.jumps_x * (0.003 / vars.screen.zoom) + ((0.003 / vars.screen.zoom) * i);
			curr_n.i = initial_n.i + vars.screen.jumps_y * (0.003 / vars.screen.zoom) + ((0.003 / vars.screen.zoom) * j);
